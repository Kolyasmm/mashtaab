...
(function() {
  // Конфигурация
  var API_BASE = 'https://vkscaler.ru';
  ...
  function startDuplication() {
    var apiToken = document.getElementById('apiTokenInput').value.trim();
    var groupId = document.getElementById('groupIdInput').value.trim();
    var copyCount = parseInt(document.getElementById('copyCountInput').value);
    ...
    var fields = 'name,objective,ad_plan_id,package_id,targetings,price,max_price,autobidding_mode,age_restrictions,date_start,date_end,budget_limit,budget_limit_day,banners,utm,enable_utm';
    
    fetch(API_BASE + '/api/vk/ad_groups/' + groupId + '.json?fields=' + fields, {
      method: 'GET',
      headers: {
    ...
    .then(function(groupData) {
      ...
      return duplicateGroups(groupData, copyCount, apiToken);
    })
  ...
  function duplicateGroups(groupData, copyCount, apiToken) {
    var successCount = 0;
    var promises = [];
    
    for (var i = 1; i <= copyCount; i++) {
      promises.push(createGroupCopy(groupData, i, apiToken));
    }
    
    return Promise.all(promises).then(function(results) {
      successCount = results.filter(function(r) { return r; }).length;
      addLog('\n🏁 Готово: ' + successCount + '/' + copyCount, 'success');
      showStatus('✅ Создано ' + successCount + ' копий!', 'success');
      document.getElementById('startBtn').disabled = false;
      document.getElementById('startBtn').textContent = '🚀 Начать дублирование';
    });
  }
  
  function createGroupCopy(groupData, copyNum, apiToken) {
    addLog('\n📋 Копия ' + copyNum + '...', 'info');
    
    var newGroupData = JSON.parse(JSON.stringify(groupData));
    newGroupData.name = groupData.name + ' (копия ' + copyNum + ')';
    ...
    return fetch(API_BASE + '/api/vk/ad_groups.json', {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer ' + apiToken,
        'Content-Type': 'application/json'
    ...
    .then(function(result) {
      ...
      updateProgress(copyNum, copyCount);
      return true;
    })
    .catch(function(error) {
      addLog('  ❌ Ошибка: ' + error.message, 'error');
